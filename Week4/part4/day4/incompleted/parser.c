...
void compileForSt(void) {
  ...
  eat(SB_ASSIGN);
  firstExpType = compileExpression();
  checkTypeEquality(varType, firstExpType);

  eat(KW_TO);
  secExpType =  compileExpression();
  checkTypeEquality(varType, secExpType);
  ...
  case TK_IDENT:
    ...
    case OBJ_VARIABLE:
      if (obj->varAttrs->type->typeClass == TP_ARRAY)
        compileIndexes(obj->varAttrs->type);
      else 
