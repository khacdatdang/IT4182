...
void compileAssignSt(void) {
  ...
  while(lookAhead->tokenType == SB_COMMA)
  {
    eat(SB_COMMA);
    m++;
    expType[m] = compileExpression();
  }

  if (count != m)
  {
    error(ERR_INVALID_EXPRESSION, currentToken->lineNo, currentToken->colNo);
  }

  checkTypeEquality(varType[0], expType[0]);
  for (int i = 0; i < count; ++i)
  {
    checkTypeEquality(varType[i], expType[i]);
  }
}
...
{
  ObjectNode *paramsListCompile = paramList;
  switch (lookAhead->tokenType)
  {
  case SB_LPAR:
    eat(SB_LPAR);
    if (paramsListCompile == NULL)
    {
      error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
    }
    compileArgument(paramsListCompile->object);
    paramsListCompile = paramsListCompile->next;
    while (lookAhead->tokenType == SB_COMMA)
    {
      eat(SB_COMMA);
      if (paramsListCompile == NULL)
    ...
    // check xem trong aguremtn có đủ phần tử không, thưa f=> sai rồi
    if (paramsListCompile != NULL)
    {
      error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
    }
...
Type *compileFactor(void)
{
  ...
  case TK_IDENT:
    ...
    case OBJ_CONSTANT:
      ...
      break;
    case OBJ_VARIABLE:
      // with variable we maybe have array
